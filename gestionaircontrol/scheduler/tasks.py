...
def schedule_availability():
    ...
    late_bookings = Booking.objects.filter(timeslot__end_time__isnull=True).filter(timeslot__start_time__lte=timezone.now()+datetime.timedelta(minutes=settings.SLOT_DURATION + 5))
